...
#define DEVICE_NAME L"\\Device\\NGSCat"
#define SYMLINK_NAME L"\\DosDevices\\NGSCat"
...
NTSTATUS InjectDll(PEPROCESS targetProcess, PUCHAR dllData, SIZE_T dllSize) {
    ...
    status = ZwAllocateVirtualMemory(ZwCurrentProcess(), &remoteBase, 0, ®ionSize,
    ...
    if (!NT_SUCCESS(status)) {
        ...
        ZwFreeVirtualMemory(ZwCurrentProcess(), &remoteBase, ®ionSize, MEM_RELEASE);
    ...
    for (WORD i = 0; i < ntHeaders->FileHeader.NumberOfSections; i++) {
        if (section[i].Characteristics & IMAGE_SCN_MEM_EXECUTE) {
            ...
            ULONG oldProtect;
            ZwProtectVirtualMemory(ZwCurrentProcess(), §ionBase, §ionSize,
